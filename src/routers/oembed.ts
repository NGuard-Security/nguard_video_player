...
router.get(
  ...
  async function (req: Request, res: Response, next: NextFunction) {
    ...
    const videoDataModule =
      videoDataModules[req.query.index ? Number(req.query.index) : 0]
    if (videoDataModule.type == 'v2_video') {
      const oembed = {
        ...
        html: `<iframe width="1280" height="720" src="https://video.sskate.me/naver/oembed.php?clubName=${clubName}&articleId=${articleId}&index=0" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowfullscreen title="${articleData.result.article.subject}"></iframe>`,
      }
