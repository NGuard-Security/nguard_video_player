...
router.get(
  ...
  async function (req: Request, res: Response, next: NextFunction) {
    if (!req.query.url || !(req.query.url as string).startsWith("https://cafe.naver.com/") || isNaN(Number(req.query.index || 0))) {
        return res.status(400).send('Bad Request')
    }

    const url = req.query.url as string

    const clubName = url.replace("https://cafe.naver.com/", "").split('/')[0]
    const articleId = url.replace("https://cafe.naver.com/", "").split('/')[1]
    ...
    if (videoDataModule.type == 'v2_video') {
        const oembed = {
            ...
            html: `<iframe width="1280" height="720" src="https://video.sskate.me/naver/${clubName}/${articleId}/${req.query.index || '0'}?feature=oembed" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowfullscreen title="${articleData.result.article.subject}"></iframe>`
        }
