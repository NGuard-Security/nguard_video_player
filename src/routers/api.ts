    ...
    try {
      ...
      if (videoDataModule?.type == 'v2_video') {
        const { data: videos } = await axios.get(
          ...
          }&nonce=${new Date().getTime()}&devt=html5_mo&prv=N&aup=N&stpb=N&cpl=ko_KR&providerEnv=real&lc=ko_KR&adi=%5B%7B%22adSystem%22%3A%22null%22%7D%5D&adu=%2F`,
        )
      ...
      if (videoDataModule?.type == 'v2_oembed') {
        const oembedSrc = articleElem
          .querySelector('iframe')
          ?.getAttribute('src')

        if (oembedSrc) {
          if (oembedSrc.startsWith('https://www.youtube.com/')) {
            const videoId = oembedSrc
